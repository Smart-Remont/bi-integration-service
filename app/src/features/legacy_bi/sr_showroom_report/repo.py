...
class SrShowroomReportRepository(BaseRepository):
    async def sr_showroom_report(self) -> object | None:
        try:
            rows = await self.call_sp("rest.sr_showroom_report", module_code=LEGACY_BI_MODULE_CODE)
