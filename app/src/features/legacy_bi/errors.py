...
class LegacyBiDatabaseError(Exception):
    def __init__(self, message: str) -> None:
        self.message = message
        super().__init__(message)


def to_legacy_bi_database_error(exc: Exception) -> LegacyBiDatabaseError:
    pg_error = find_postgres_error(exc)
    if pg_error is not None:
        return LegacyBiDatabaseError(clean_postgres_error_message(pg_error))
    return LegacyBiDatabaseError(str(exc))


def legacy_bi_error_code(message: str) -> str:
    """
