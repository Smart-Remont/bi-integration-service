...
class RequestEventRepository(BaseRepository):
    async def ddu_request_event(self, payload: dict[str, object]) -> int:
        try:
            rows = await self.call_sp(
                "rest.ddu__request_event",
                json.dumps(payload),
        ...
        if client_request_id is None:
            raise BigIntegrationDatabaseError("Пустой ответ от rest.ddu__request_event")
