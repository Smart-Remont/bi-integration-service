...
class RequestCreateRepository(BaseRepository):
    async def ddu_create_request(self, payload: dict[str, object]) -> int:
        try:
            rows = await self.call_sp(
                "rest.ddu__create_request",
                json.dumps(payload),
        ...
        if client_request_id is None:
            raise BigIntegrationDatabaseError("Пустой ответ от rest.ddu__create_request")
